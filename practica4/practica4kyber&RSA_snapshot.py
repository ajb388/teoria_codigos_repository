...
def validarUsuario(contraseña):
    ...
    with open(certificado, 'r') as file:
        ...
        if not certificate.strip():
            messagebox.showerror("Error", "El certificado no es correcto.")
        else:
            leerUsuariosApp()
                ...
                if lista_user["user"] == usuario:
                    try:
                        ...
                        if tipo_clave.has_private():
                            messagebox.showinfo("Información", "Clave privada")
                        else:
                            try:
                                ...
                                messagebox.showerror("Error", "Usuario no validado")
                                break
                    except ValueError:
                        try:
                            public_key_user_base64 = certificate['public_key']
...
def login():
    ...
    logo_path = os.path.join(directorio_base, "img", "logo.jpg")
    ...
    label_logo = ctk.CTkLabel(login_ventana, image=logo_image, text="")
