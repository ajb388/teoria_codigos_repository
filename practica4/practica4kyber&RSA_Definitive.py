...
def encriptarArchivosUsers(lista_usuarios_encriptan, lista_rutas_archivos):
    ...
    for archivo in lista_rutas_archivos:
        ...
        hash_object = SHA256.new()
        try:
            datos_cifrados_str = datos_cifrados.decode('utf-8')
        except UnicodeDecodeError:
            datos_cifrados_str = base64.b64encode(datos_cifrados).decode('utf-8')
        datos = ' '.join(lista_claves_cifradas) + " " + datos_cifrados_str + " " + extension
        hash_object.update(datos.encode('utf-8'))
        hash_bytes = hash_object.digest()
        desencriptarPrivate_app()
        signature = Dilithium2.sign(private_app, hash_bytes)
        ...
        with open(rutasalida2, 'wb') as f:
            ...
            f.write(len(firma_bytes).to_bytes(2,'big'))
            f.write(firma_bytes)
...
def desencriptarArchivosUsers(lista_rutas_archivos):
    ...
    for archivo in lista_rutas_archivos:
        ...
        with open (archivo, 'rb') as file:
            ...
            len_firma = int.from_bytes(file.read(2),'big')
            firma = file.read(len_firma).decode('utf-8')
            ...
            #TODO zona de validación de firma
            try:
                datos_cifrados_str = datos_cifrados.decode('utf-8')
            except UnicodeDecodeError:
                datos_cifrados_str = base64.b64encode(datos_cifrados).decode('utf-8')
            comprobante = ' '.join(lista_claves_cifradas) + " " + datos_cifrados_str + " " + extension
            hash_comprobante = SHA256.new(comprobante.encode('utf-8'))
            hash_comprobante_bytes = hash_comprobante.digest()
            firm=bytes.fromhex(firma)
            Dilithium2.verify(public_app, hash_comprobante_bytes, firm)
            print("Archivo validado con firma")
