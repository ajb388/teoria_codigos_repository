...
# Ruta base para asegurarse de guardar claves en el mismo directorio del script
ruta_base = os.path.dirname(os.path.abspath(__file__))
archivo_claves = os.path.join(ruta_base, "clavesApp.txt")

#Generar claves de la App
def gestionar_claves():
     # Crear la carpeta 'main' en el mismo directorio que el script
    carpeta_main = os.path.join(ruta_base, "main")
    if not os.path.exists(carpeta_main):
        os.makedirs(carpeta_main)  # Crear la carpeta si no existe
    # Definir las rutas para los archivos de claves
    archivo_clave_publica = os.path.join(carpeta_main, "clave_publicaApp.pem")
    archivo_clave_privada = os.path.join(carpeta_main, "clave_privadaApp.pem")

    # Verificar si ambos archivos existen
    if os.path.exists(archivo_clave_publica) and os.path.exists(archivo_clave_privada):
        print("Los archivos de claves ya existen. Cargando claves...")
        with open(archivo_clave_publica, "r") as f:
            clave_publica = f.read()
        with open(archivo_clave_privada, "r") as f:
            clave_privada = f.read()
        print("Claves cargadas exitosamente.")
    else:
        print("Los archivos de claves no existen. Generando nuevas claves...")
        # Generar claves RSA
        from Crypto.PublicKey import RSA
        claves = RSA.generate(2048)
        clave_privada = claves.export_key().decode("utf-8")
        clave_publica = claves.publickey().export_key().decode("utf-8")

        # Guardar las claves en sus respectivos archivos
        with open(archivo_clave_publica, "w") as f:
            f.write(clave_publica)
        with open(archivo_clave_privada, "w") as f:
            f.write(clave_privada)
        print("Nuevas claves generadas y guardadas exitosamente.")

    return clave_publica, clave_privada


# Llamar a la función para asegurarse de que las claves estén disponibles al inicio
clave_publica, clave_privada = gestionar_claves()
...
directorio_base = os.path.join(os.path.dirname(__file__), "main")
...
def login():
    def registrar_usuario():
        def guardar_usuario():
            ...
            with open(ruta_usuarios, "a") as f:
                f.write(f"{nuevo_usuario},{hash_contrasena}\n")
    ...
    def verificar_credenciales():
        ...
        hash_contrasena_ingresada = hashlib.sha256(contrasena.encode()).hexdigest()
        ...
        try:
            with open(os.path.join(directorio_base, "usuarios.txt"), "r") as f:
                ...
                for usuario_linea in usuarios:
                    if "," in usuario_linea:
                        usuario_guardado, hash_contrasena_guardada = usuario_linea.strip().split(",")
                        # Comparar usuario y hash de la contraseña
                        if usuario == usuario_guardado and hash_contrasena_ingresada == hash_contrasena_guardada:
